...
function! zencoding#lang#default#findTokens(str)
  let str = a:str
...
function! zencoding#lang#default#parseIntoTree(abbr, type)
  let abbr = a:abbr
  let type = a:type
...
function! zencoding#lang#default#toString(settings, current, type, inline, filters, itemno, indent)
...
function! zencoding#lang#default#imageSize()
  ...
  let current = zencoding#lang#default#parseTag(content)
...
function! zencoding#lang#default#encodeImage()
  let img_region = zencoding#util#searchRegion('<img\s', '>')
  if !zencoding#util#regionIsValid(img_region) || !zencoding#util#cursorInRegion(img_region)
    return
  endif
...
function! zencoding#lang#default#parseTag(tag)
...
function! zencoding#lang#default#toggleComment()
  let orgpos = getpos('.')
  let curpos = getpos('.')
  let mx = '<\%#[^>]*>'
  while 1
...
function! zencoding#lang#default#balanceTag(flag) range
...
function! zencoding#lang#default#moveNextPrev(flag)
...
function! zencoding#lang#default#splitJoinTag()
  let curpos = getpos('.')
  while 1
...
function! zencoding#lang#default#removeTag()
  let curpos = getpos('.')
  while 1
    let mx = '<\(/\{0,1}[a-zA-Z][a-zA-Z0-9:_\-]*\)[^>]*>'
    let pos1 = searchpos(mx, 'bcnW')
