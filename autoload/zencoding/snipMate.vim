...
let s:c.read_snippets_cached = get(s:c, 'read_snippets_cached', {'func' : function('zencoding#snipMate#readSnippetsFile'), 'version': 3, 'use_file_cache': s:c.cache_parsed_snippets_on_disk})

" if filetype is objc, cpp, cs or cu also append snippets from scope 'c'
" you can add multiple by separating scopes by ',', see zencoding#snipMate#addScopeAliases
...
fun! zencoding#snipMate#filename(...)
  let filename = expand('%:t:r')
  if filename == '' | return a:0 == 2 ? a:2 : '' | endif
  return !a:0 || a:1 == '' ? filename : substitute(a:1, '$1', filename, 'g')
endf

fun! zencoding#snipMate#removeSnippet()
...
fun! zencoding#snipMate#expandSnip(snip, col)
  let lnum = line('.') | let col = a:col

  let snippet = zencoding#snipMate#processSnippet(a:snip)
  ...
  let [g:snipPos, s:snipLen] = zencoding#snipMate#buildTabStops(snippet, lnum, col - indent, indent)

  " Remove $cursor$ if the g:snipPos exist
  if s:snipLen
  ...
  call setline(lnum, snipLines[0])
  call append(lnum, map(snipLines[1:], "'".strpart(line, 0, indent - 1)."'.v:val"))

  if s:snipLen
    aug snipMateAutocmds
      au CursorMovedI * call zencoding#snipMate#updateChangedSnip(0)
      au InsertEnter * call zencoding#snipMate#updateChangedSnip(1)
    aug END
    ...
    if g:snipPos[s:curPos][2] != -1 | return zencoding#snipMate#selectWord() | endif
  else
...
" Prepare snippet to be processed by zencoding#snipMate#buildTabStops
fun! zencoding#snipMate#processSnippet(snip)
  let snippet = a:snip
...
fun! zencoding#snipMate#count(haystack, needle)
  let counter = 0
...
fun! zencoding#snipMate#buildTabStops(snip, lnum, col, indent)
  let snipPos = []
  let i = 1
  ...
  while a:snip =~ s:d.'{'.i
    ...
    let snipPos[j][0] = a:lnum + zencoding#snipMate#count(beforeTabStop, "\n")
    ...
    if withoutVars =~ ''.s:d .'{'.i.':'
      ...
      while match(withoutOthers, ''.s:d .''.i.'\(\D\|$\)') != -1
        ...
        let snipPos[j][3][-1][0] = a:lnum + zencoding#snipMate#count(beforeMark, "\n")
...
fun! zencoding#snipMate#jumpTabStop(backwards)
  ...
  if exists('s:update')
    call zencoding#snipMate#updatePlaceholderTabStops()
  else
    call zencoding#snipMate#updateTabStops()
  endif
  ...
  if s:curPos == s:snipLen
    ...
    call zencoding#snipMate#removeSnippet()
    return sMode ? "\<tab>" : zencoding#snipMate#triggerSnippet()
  endif
  ...
  return g:snipPos[s:curPos][2] == -1 ? '' : zencoding#snipMate#selectWord()
endf

fun! zencoding#snipMate#updatePlaceholderTabStops()
...
fun! zencoding#snipMate#updateTabStops()
...
fun! zencoding#snipMate#selectWord()
...
fun! zencoding#snipMate#updateChangedSnip(entering)
  ...
  elseif exists('s:update') " If modifying a placeholder
    ...
    if line('.') != g:snipPos[s:curPos][0] || col < s:startCol ||
      ...
      return zencoding#snipMate#removeSnippet()
    endif

    call zencoding#snipMate#updateVars()
    let s:prevLen[1] = col('$')
...
fun! zencoding#snipMate#updateVars()
...
fun! zencoding#snipMate#setByPath(dict, path, value)
  let d = a:dict
  for p in a:path[:-2]
...
fun! zencoding#snipMate#readSnippetsFile(file)
  let result = []
...
fun! zencoding#snipMate#addScopeAliases(list)
  let did = {}
...
fun! zencoding#snipMate#glob(dir,  file)
  let f = a:dir.a:file
...
fun! zencoding#snipMate#getSnippetFiles(mustExist, scopes, trigger)
  let paths = funcref#Call(s:c.snippet_dirs)

  let result = {}
  let scopes = zencoding#snipMate#addScopeAliases(a:scopes)
  ...
  for r in paths
    ...
    for f in zencoding#snipMate#glob(r.'/snippets/'.scope,'/'.a:trigger.'.snippet')
...
fun! zencoding#snipMate#evalGuard(guard)
...
fun! zencoding#snipMate#defaultPool(scopes, trigger, result)
  let triggerR = substitute(a:trigger,'*','.*','g')
  for [f,opts] in items(zencoding#snipMate#getSnippetFiles(1, a:scopes, a:trigger))
    if opts.type == 'snippets'
      for [trigger, name, contents, guard] in cached_file_contents#CachedFileContents(f, s:c.read_snippets_cached, 0)
        ...
        if zencoding#snipMate#evalGuard(guard)
          call zencoding#snipMate#setByPath(a:result, [trigger, opts.name_prefix.' '.name], contents)
        endif
      endfor
    elseif opts.type == 'snippet'
      call zencoding#snipMate#setByPath(a:result, [opts.trigger, opts.name_prefix.' '.opts.name], funcref#Function('return readfile('.string(f).')'))
    else
      throw "unexpected"
    endif
  endfor
...
"
fun! zencoding#snipMate#getSnippets(scopes, trigger)
  let result = {}
...
fun! zencoding#snipMate#retabSnip() range
...
fun! zencoding#snipMate#openSnippetFiles()
  let dict = zencoding#snipMate#getSnippetFiles(0, zencoding#snipMate#scopesByFile(), '*')
...
fun! zencoding#snipMate#scopesByFile()
  " duplicates are removed in AddScopeAliases
  return filter(funcref#Call(s:c.get_scopes), "v:val != ''")
endf

" used by both: completion and insert snippet
fun! zencoding#snipMate#GetSnippetsForWordBelowCursor(word, suffix, break_on_first_match)
  ...
  for word in lookups
    ...
    for [k,snippetD] in items(funcref#Call(s:c['get_snippets'], [zencoding#snipMate#scopesByFile(), word]))
      if a:suffix == ''
...
fun! zencoding#snipMate#chooseSnippet(snippets)
  let snippet = []
...
fun! zencoding#snipMate#showAvailableSnips()
  let line  = getline('.')
  let col   = col('.')
  ...
  let snippet_triggers = map(zencoding#snipMate#GetSnippetsForWordBelowCursor(word, '*', 0),'v:val[0]')
...
fun! zencoding#snipMate#triggerSnippet()
  ...
  if exists('g:snipPos') | return zencoding#snipMate#jumpTabStop(0) | endif

  let word = matchstr(getline('.'), '\S\+\%'.col('.').'c')
  let list = zencoding#snipMate#GetSnippetsForWordBelowCursor(word, '',  1)
  if empty(list)
    let snippet = ''
  else
    let [trigger, snippetD] = list[0]

    let s = zencoding#snipMate#chooseSnippet(snippetD)
    if type(s) == type([])
      let snippet = join(s, "\n")
    else
      let snippet = s
    ...
    return zencoding#snipMate#expandSnip(snippet, col)
  endif
  ...
  return word == ''
    \ ? "\<tab>"
    \ : "\<c-r>=zencoding#snipMate#showAvailableSnips()\<cr>"
endf


fun! zencoding#snipMate#backwardsSnippet()
  if exists('g:snipPos') | return zencoding#snipMate#jumpTabStop(1) | endif
